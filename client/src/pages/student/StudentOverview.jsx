...
export default function StudentOverview() {
  ...
  const trendData = Object.values(dateMap).sort((a, b) => a.month.localeCompare(b.month)).map(d => ({
    month: d.month,
    ...
    return (
      <div className="page-enter">
        <div className="mb-6">
          ...
          <p className="text-slate-500 text-sm mt-0.5">Detailed subject-wise analytics</p>
        </div>
        <EmptyState icon="📊" title="No data available" desc="Your attendance analytics will appear here once your faculty marks attendance." />
      </div>
    );
  }
  ...
  return (
    <div className="page-enter">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-800">Attendance Overview</h1>
        <p className="text-slate-500 text-sm mt-0.5">Subject-wise analytics and attendance trends</p>
      </div>
      ...
      {trendData.length > 1 && (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 mb-5">
          <h3 className="text-base font-semibold text-slate-800 mb-4">Monthly Attendance Trend</h3>
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
