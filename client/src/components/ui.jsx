...
export function TabNav({ tabs, active, onChange }) {
  return (
    <div className="flex bg-slate-100 p-1 rounded-xl mb-6 w-full gap-1">
      {tabs.map(t => (
        <button
          key={t.id}
          onClick={() => onChange(t.id)}
          className={`flex-1 py-2.5 rounded-lg font-bold text-xs sm:text-sm transition-all duration-200 flex items-center justify-center gap-1.5 ${
            active === t.id
